...
function getTokenFromResult(result) {
	return result?.token || result?.accessToken || result?.data?.token || result?.data?.accessToken || '';
}
...
function Login() {
	...
	const handleSubmit = async (event) => {
		...
		try {
			...
			isAdmin = decoded.role === 'admin' || (Array.isArray(decoded.roles) && decoded.roles.includes('admin'));
		} catch {
			isAdmin = false;
		}

		login(email, token);
		navigate(isAdmin ? '/hotel-admin' : '/home', { replace: true });
	};
